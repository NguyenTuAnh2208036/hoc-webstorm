function getTarget(e) {                    // Declore function
...
function itemDone(e) {                    // Declore function
    // Remove item from the list
    var target, elParent, elGrandparent;  // Declore variablue
    target = getTarget(e);                // Get the item clicked link

    /*
    The bock used the following code - but it had two shortcomings
    - Clicking between list item would remove the whole list
    - Clicking on italic next would remove the link - not the list item
...
if (el.addEventListener) {                                   // If event listeners work
    el.addEventListener('click', function (e:) {  // Add listeners work
        itemDone(e);                                         // It cool itemDone()
